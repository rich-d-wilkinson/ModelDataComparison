...
#function to make inference about the geological observations using a GMRF.
...
climateFit_gmrf <- function(Obs_set, obs, manifold, smooth, stretch ){
  # OUTPUT:
  mesh <- paste0(manifold,"-",stretch)
  outdir <- paste0("Output/O-",Obs_set,"_ME-",mesh,"_SM-",smooth)
  ...
  Res <- array(0,dim=c(2964,5,length(obs)))
  for (j in 1:length(obs)) {
    ...
    # Find observation locations in mesh (i.e. find the basis function values for each observation)
    ...
    # InferenceL returns the mean, covariance and precision of the weights given the observations
    ...
    # EXTRACT RESULLTS
    Res_pre <- Inf_data$Res
    Res_bind <- cbind(Res_pre,data.frame(x=p_lon_lat[,1],y=p_lon_lat[,2],val=x_sim))
    print(dim(Res_bind))
...
test <- climateFit_gmrf(Obs_set, obs, manifold, smooth, stretch)
...
climateModelComparison <- function(Model_set, models, model_grid,  ){
