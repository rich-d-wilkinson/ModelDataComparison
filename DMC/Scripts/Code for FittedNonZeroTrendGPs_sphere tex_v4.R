...
ptm2 <- proc.time()
comp.NegLogLik <- composite.nll(GCM.param, residual.GCM.data,c(4,4),"Matern", m.error.GCM.vec, "NegLogLik") #Agrees with real.NegLogLik 
proc.time() - ptm2
#
#neg. log-likelihood:
...
maternParam.test <- c( 0.5, 10, 3, 0.1)
expParam.test <- c( 0.5, 5, 0.01)
...
for(j in 1:8){ #loop over the GCMs
  ...
  start1 <- Sys.time()
  comp.NegLogLik <- composite.nll(maternParam.test[2:4],residual.GCM.data,c(2,1),"Exponential", m.error.GCM.vec, "NegLogLik") 
  end1 <- Sys.time()
  time1 <- end1 - start1
  
  start2 <- Sys.time()
  comp1.NegLogLik <- composite.nll(maternParam.test[2:4], residual.GCM.data,c(7,7),"Exponential", m.error.GCM.vec, "NegLogLik") 
  #Agrees with real.NegLogLik 
  end2 <- Sys.time()
  time2 <- end2 - start2
  
  start3 <- Sys.time()
  comp2.NegLogLik <- composite.nll(maternParam.test[2:4], residual.GCM.data,c(9,9),"Exponential", m.error.GCM.vec, "NegLogLik") 
  #Agrees with real.NegLogLik 
  end3 <- Sys.time()
  time3 <- end3 - start3
  
  start4 <- Sys.time()
  comp3.NegLogLik <- composite.nll(maternParam.test[2:4], residual.GCM.data,c(12,12),"Exponential", m.error.GCM.vec, "NegLogLik") 
  #Agrees with real.NegLogLik 
  end4 <- Sys.time()
  time4 <- end4 - start4
...
#
# locations are given from the GCM output loop.
#
...
meas.error <- Obs[,4]^2 
krige.output <- krige0(residual_Obs ~ 1, residual.sp, location.GCM.points, model = expCov.krige, par.vec = maternParam.test[2:4] , m.error = meas.error )
...
# Calculate Full Likelihood using composite code with 1x2 blocks:
start1 <- Sys.time()
comp.krige.NegLogLik <- composite.nll(maternParam.test[2:4], krige.data, c(2,1), "Exponential", m.error.GCM.vec, "NegLogLik") 
#Agrees with real.NegLogLik 
end1 <- Sys.time()
time1 <- end1 - start1

start2 <- Sys.time()
comp1.krige.NegLogLik <- composite.nll(maternParam.test[2:4], krige.data,c(7,7),"Exponential", m.error.GCM.vec, "NegLogLik") 
#Agrees with real.NegLogLik 
end2 <- Sys.time()
time2 <- end2 - start2

start3 <- Sys.time()
comp2.krige.NegLogLik <- composite.nll(maternParam.test[2:4],  krige.data,c(9,9),"Exponential",m.error.GCM.vec, "NegLogLik") 
#Agrees with real.NegLogLik 
end3 <- Sys.time()
time3 <- end3 - start3

start4 <- Sys.time()
comp3.krige.NegLogLik <- composite.nll(maternParam.test[2:4],  krige.data,c(12,12),"Exponential",m.error.GCM.vec, "NegLogLik") 
#Agrees with real.NegLogLik 
end4 <- Sys.time()
time4 <- end4 - start4

lik.row <- c(comp.krige.NegLogLik, comp1.krige.NegLogLik, comp2.krige.NegLogLik, comp3.krige.NegLogLik)
...
#
#
# 1. draw two independent samples from the GP:
krige.output.1 <- krige0(residual_Obs ~ 1, residual.sp, location.GCM.points, model = expCov.krige, par.vec = maternParam[2:4] , m.error = meas.error )
krige.output.2 <- krige0(residual_Obs ~ 1, residual.sp, location.GCM.points, model = expCov.krige, par.vec = maternParam[2:4] , m.error = meas.error )
#krige0 does not produce variance predictions at the locations. Will have to estimate this manually if we want to use the C.I. metric. 
...
for(j in 1:3){ #loop over the GCMs
  Model_df <- Read_model_data(modelpath = paste0("Model_data/",Model_set,"/",models[j],".txt"),
                              indexpath = paste0("Model_data/",Model_set,"/mask.txt"),
                              model_grid, plot_it=T) 
  ...
  gcm.crps[j] <- mCRPS.eval(residual_GCM, krige.data.crps) 
