...
vgm.sample <- variogram(residual_Obs ~ 1, data = y_obs.sp, cutoff = 5000, width = 500 )  
plot(vgm.sample)
dev.copy(pdf,'sampleVGM_residual_lm.pdf')
dev.off()

#MLE for covariance marix: theta = (nu, rho, sigma, nugget)
n <- length(residual_Obs)
dist.mat <- geoDistance(locations,locations)
param.lower <- c(0.01,0.01,0.01,0)
#needs upper limit for nu <= 1/2 too.
param.upper <- c(1/2,10000,100000,1000) #for validity on the sphere.
ini <- c(0.5,15,20,0) #how do we find initial values?
...
test.length <- c(1:5)
pred.data <- cbind(data.pred[test.length,1:2], residual_Obs[test.length])
