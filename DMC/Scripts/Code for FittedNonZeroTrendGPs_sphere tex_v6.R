## Code accompanying the tex file: FittingNonZeroTrendGPs_sphere (May 2017)
# version 6 : Fixed mistake in covariance matrix calculation.
# ---------------------------------------------------
# ---------- Pre-Amble: -----------------------------
...
initial.MLE <- array(c(0.5,0.5,0.5,2000,2500,3500,1,1,1,1,1,1), dim=c(3,4)) #initial parameter values for optimisation.
...
#
data.Obs <- cbind(locations, residual_Obs)
...
#
covarianceMLE <- covariance.MLE(initial.MLE[,2:4], residual_Obs, dist.mat.obs, matrixFunction, meas.error)
lower.b <- c(500,0.01,0.001)
upper.b <- c(15000, 3, 1)
#covarianceMLE <- covariance.MLE.PSO(array(1, dim=c(1,3)), residual_Obs, lower.b, upper.b, dist.mat.obs, matrixFunction, meas.error)
...
location.GCM <- Model_df.full[index,2:3] ## from random locations
...
K.joint <- expCov(dist.mat.joint, covarianceMLE$par[1:2], m.error.GCM.vec)
cov.Obs <- expCov(dist.mat.obs, covarianceMLE$par, meas.error)

Cov.mat <- expCov(dist.mat.GCM, covarianceMLE$par[1:2], m.error.GCM.vec) + K.joint%*%solve(cov.Obs)%*%t(K.joint)
if (det(Cov.mat) < 0.000000000000001){
  print("Singular")
  Cov.mat <- Cov.mat + 0.5*diag(dim(Cov.mat)[1])
}
mean.vec <- K.joint%*%solve(cov.Obs)%*%( residual_Obs ) 
...
for(j in 1:8){ #loop over the GCMs
  ...
  # start1 <- Sys.time()
   output.lik[j,1] <- composite.nll(covarianceMLE$par, residual.GCM.data, block.1, "Exponential", 0*m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs, cov.Obs)  
  # end1 <- Sys.time()
  # time1 <- end1 - start1
  # # NOte: Block composition c(2,1) is equivalent to the usual log-likelihood
  # start2 <- Sys.time()
   output.lik[j,2] <- composite.nll(covarianceMLE$par, residual.GCM.data, block.2, "Exponential", 0*m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs, cov.Obs)   
  # end2 <- Sys.time()
  # time2 <- end2 - start2
  # #
  # start3 <- Sys.time()
   output.lik[j,3] <- composite.nll(covarianceMLE$par, residual.GCM.data, block.3, "Exponential", 0*m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs, cov.Obs) 
  ...
  #output.lik[j,1] <- composite.nll(covarianceMLE$par, residual.GCM.data, c(2,1), "Exponential", 0*m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs, cov.Obs) 
  #output.lik[j,2] <- 1/2*log(det(Cov.mat)) + 1/2*t(residual_GCM[,j] - mean.vec)%*%solve(Cov.mat)%*%(residual_GCM[,j] - mean.vec)
}
...
krige.output <- krige0(residual_Obs ~ 1, residual.sp, location.GCM.points, model = expCov.krige, par.vec = covarianceMLE$par , m.error = meas.error )
...
# start1 <- Sys.time()
# comp.krige.NegLogLik <- composite.nll(covarianceMLE$par, krige.data, block.1, "Exponential", m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs)  
# end1 <- Sys.time()
# time1 <- end1 - start1
# #
# start2 <- Sys.time()
# comp1.krige.NegLogLik <- composite.nll(covarianceMLE$par, krige.data,block.2,"Exponential", m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs)  
# end2 <- Sys.time()
# time2 <- end2 - start2
# #
# start3 <- Sys.time()
# comp2.krige.NegLogLik <- composite.nll(covarianceMLE$par,  krige.data,block.3,"Exponential", m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs)  
# end3 <- Sys.time()
# time3 <- end3 - start3
# #
...
#
#full log-likelihood:
output.lik[9,1] <- composite.nll(covarianceMLE$par[1:2], krige.data, c(2,1), "Exponential", m.error.GCM.vec, "NegLogLik", dist.mat.obs, data.Obs, cov.Obs)
output.lik[9,2] <- 1/2*log(det(Cov.mat)) + 1/2*t(krige.output - mean.vec)%*%solve(Cov.mat)%*%(krige.output - mean.vec)
#
