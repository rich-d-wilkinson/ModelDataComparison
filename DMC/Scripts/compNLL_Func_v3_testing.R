...
composite.nll <- function(Cov.par, data, m, cov.model, m.error, scoreFunc, dist.mat.obs, data.Obs, cov.Obs){
  #Input:
  # Cov.par - vector of covariance matrix function parameters
  # data - GCM data : columns are (lat, long, val)
  # m - block size vector c(n_1,n_2)
  # cov.model - One of "Matern", "Exponential" or "Spherical"
  # m.error - meaurement errors: vector
  # scoreFunc - score function
  # dist.mat.obs - matrix of distances for the observations
  # data.obs - matrix where columns are (lat, long, val) from the observation data
  # 
  ...
  if(scoreFunc == "NegLogLik"){
    ...
    for(i in 1:dim(N_adj_arrow)[2]){ 
            nll.out <- nll.out + nll.calc(data, D_index[(D_Change[N_adj_arrow[1,i]]+1):(D_Change[N_adj_arrow[1,i]+1]-1)],
                                      D_index[(D_Change[N_adj_arrow[2,i]]+1):(D_Change[N_adj_arrow[2,i]+1]-1)],cov.model,Cov.par, m.error, dist.mat.obs, data.Obs, cov.Obs)
     }
    score.out <- nll.out
  }
...
blockCovariance <- function(D1,D2,data,model,cov.param, m.error, dist.mat.obs, data.Obs, cov.Obs){
  ...
  dist.mat.joint <- geoDistance(S_all, data.Obs[,1:2])
  ...
  if( model == "Matern"){
    K.joint <- maternCov(dist.mat.joint, cov.param, array(0, dim=c(1 ,dim(dist.mat.joint)[1])))
    Cov.mat <- maternCov(dist.mat, cov.param, m.error.blocks) + K.joint%*%solve(cov.Obs)%*%t(K.joint)
  ...
  #}
    ...
    K.joint <- expCov(dist.mat.joint, cov.param, array(0, dim=c(1 ,dim(dist.mat.joint)[1])))
    Cov.mat <- expCov(dist.mat, cov.param, m.error.blocks) + K.joint%*%solve(cov.Obs)%*%t(K.joint)
    if (det(Cov.mat) < 0.000000000000001){
...
blockMean <- function(D1,D2,data,model,cov.param, m.error, dist.mat.obs, data.Obs, cov.Obs){
  #distance calculation:
  #1. sites for each block?
  S1 <- data[D1,1:2] #coordinates of sites in block 1
  S2 <- data[D2,1:2] 
  ...
  mean.vec <- K.joint%*%solve(cov.Obs)%*%( data.Obs[,3] ) 
...
nll.calc <- function(data, D1, D2, cov.model, cov.par, m.error, dist.mat.obs, data.Obs, cov.Obs){
  Cov <-  blockCovariance(D1,D2,data,cov.model,cov.par, m.error, dist.mat.obs, data.Obs, cov.Obs )
  Mean.vec <- blockMean(D1, D2, data, cov.model, cov.par, m.error, dist.mat.obs, data.Obs, cov.Obs )
  ...
  if( det.Cov < 0.0000000000000000000001 ){
    ...
    #print("B")
    #print(det.adj.Cov)
    out <- 1/2*log(det.adj.Cov) + 1/2*t(data_nll - Mean.vec)%*%solve(adj.Cov)%*%(data_nll - Mean.vec)
  }
  else{
    out <-  1/2*log(det.Cov) + 1/2*t(data_nll - Mean.vec)%*%solve(Cov)%*%(data_nll - Mean.vec)
  }
  #print(out)
...
covariance.neg.lik.calc.NM <- function(cov.param, data, m.error, dist.mat, model){
  #For Nelder-Mead optimisaion:
    ...
    if( model == "Matern"){
      ...
      cov.param.pos <- cbind(exp(cov.param[1:2]), cov.param[3:length(cov.param)])
      Cov <- maternCov(dist.mat, cov.param.pos, m.error)
      if(cos.param.pos[1] > 1/2){
      ...
      #Exponentiate parameters to get around constraint problem:
      cov.param.pos <- cbind(exp(cov.param[1]), cov.param[2:length(cov.param)])
...
covariance.MLE.NelderMead <- function(initial, residual_Obs, dist.mat, matrixFunction, meas.error){
  ...
  for(m in 1:M){
    if( matrixFunction == "Matern_nug"){
      ...
      mle.output <- optim(initial[m,], covariance.neg.lik.calc.NM, data = residual_Obs, m.error =  meas.error, dist.mat = dist.mat, model = "Exponential", method="Nelder-Mead")
    }
    
    else if (matrixFunction == "Exponential"){
      # iv) Exponential without nugget:

      mle.output <- optim(initial[m,], covariance.neg.lik.calc.NM, data = residual_Obs, m.error =  meas.error, dist.mat = dist.mat, model = "Exponential", method="Nelder-Mead") #,control=list(trace=TRUE))
    }
